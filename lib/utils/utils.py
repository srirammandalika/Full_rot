...
def loadModelWeight(Model, FreezeWeight, PreTrainedWeight, 
                    PreTrained=1, DistMode=False, DropLast=False, SkipSELayer=False):
    IgnoreStrList = ["running_mean", "running_var", "num_batches_tracked"] 
    ...
    if PreTrained == 1 or PreTrained == 3:
        ...
        if len(PretrainedDict) == 1:
            ...
            PretrainedDict = PretrainedDict['model_state'] 
        ...
        for _ in range(MaxLen):
            ...
            if OldVal.shape == NewVal.shape:
                if not (SkipSELayer and "selayer" in NewKey.lower()):
                    PretrainedDict[NewKey] = PretrainedDict.pop(OldKey)
                    Count += 1
            elif LenFlag:
                if "selayer" in NewKey.lower():
                    IdxOld -= 1
                else:
                    IdxNew -= 1
            IdxNew += 1
...
def deviceInit(opt):
    DataParallel = False
    MPS_AVAI = torch.backends.mps.is_available()
    if MPS_AVAI:
        DeviceStr = 'mps'
    else:
        DeviceStr = 'cpu'
