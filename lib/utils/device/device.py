...
DataParallel = False  # Modify this if you need DataParallel for CPU or other setups
...
def workerManager(BatchSize, NumWorkers=None, IsPrint=True):
    ...
    PinMemory = True if MPS_AVAI else False
    if IsPrint:
        print("We use [%s/%d] workers, and pin memory is %s" % (NumWorkers, NUM_PROC, PinMemory))
    return PinMemory, NumWorkers
